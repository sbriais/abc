...
let bisim_add p q d b =
  let (p,q) = (match compare p q with
		   t when t <= 0 -> (p,q)
		 | _ -> (q,p)) in
    Bisimulations.add (p,q,d) b

let bisim_exists p q d r =
  let (p,q) = (match compare p q with
		   t when t <= 0 -> (p,q)
		 | _ -> (q,p)) in
...
let extbisim_add p q d tp tq b =
  let (p,q,tp,tq) = (match compare p q with
...
let bisim_empty p q d tp tq =
  not_bisim := extbisim_add p q d tp tq (!not_bisim);
  Bisimulations.empty

let bisim_neq p q d =
  let tp = ref [] and tq = ref [] in
  let (p,q,rev) = (match compare p q with
...
let rec add env s_comm w_comm (* trace_p_strong trace_q_weak *) (* trace_p_weak trace_q_strong *) p q d r =
  let (b,(tp,tq)) = bisim_neq p q d in
  if b then 
    begin
(*
      Format.print_string "exit 1";Format.print_newline();
*)
      ((bisim_empty p q d tp tq),(tp,tq))
    end
  else
    ...
    (* if (bisim_exists p q d r) || (compare p q = 0) then bisim_add p q d r *)
    if (bisim_exists p q d r) then 
      begin
(*
...
*)
	...
	let ar_p = arity p and ar_q = arity q in
	  if (ar_p = 0) && (ar_q = 0) then
		...
		let (r',(tp,tq)) = matches env s_comm w_comm (* trace_p_strong trace_q_weak *) (* trace_p_weak trace_q_strong *) pc qcw d (bisim_add p q d r) in
		  if Bisimulations.is_empty r' then ((bisim_empty p q d tp tq),(tp,tq))
		  else
		    begin
			...
			let (r'',(tq,tp)) = matches env s_comm w_comm (* trace_q_strong trace_p_weak *) (* trace_q_weak trace_p_strong *) qc pcw d r' in
			  if Bisimulations.is_empty r'' then ((bisim_empty p q d tp tq),(tp,tq))
			  else
			    begin
			      (r'',(tp,tq))
		...
		begin
		  ...
		  and (nq,q') = split_abstraction env 0 q in
		    ...
		    and q'' = substitute_agent sigma q' in
		      add env s_comm w_comm (* trace_p_strong trace_q_weak *) (* trace_p_weak trace_q_strong *) p'' q'' d (bisim_add p q d r)
		end
	      else
...
and matches env s_comm w_comm (* trace_p_strong trace_q_weak *) (* trace_p_weak trace_q_strong *) p_comm q_comm d  r =
	  ...
	  let tp_tail = ref [] and tq_tail = ref [] in
	    (try
			...
			if (compare_action alpha' (substitute_action sigma_m 0 beta) = 0) then 
			  begin
			    let q' = substitute_agent sigma_m q in
			      ...
			      let (r',(tp',tq')) = add env s_comm w_comm (* trace_p' (trace_q_weak#newSon (alpha',q')) *) (* trace_p_weak trace_q_strong *) p' q' d' r in
				if (Bisimulations.is_empty r') then 
				  begin
				    tp_tail := tp';
				    tq_tail := tq';
		 ...
		 FoundB(r') -> 
		   ...
		   matches env s_comm w_comm (* trace_p_strong trace_q_weak *) (* trace_p_weak trace_q_strong *) (Commitments.remove (m,alpha,p) p_comm) q_comm d r')
	end
...
let eq env p q d =
  clear_all ();
  let (res,t) = add env commitments commitments (* trace_left_strong trace_right_weak *) (* trace_left_weak trace_right_strong *) (standard_form env 0 p) (standard_form env 0 q) d Bisimulations.empty in
    (Bisimulations.cardinal res,res,t)

let weq env p q d =
  clear_all ();
  let (res,t) = add env commitments weak_commitments (* trace_left_strong trace_right_weak *) (* trace_left_weak trace_right_strong *) (standard_form env 0 p) (standard_form env 0 q) d Bisimulations.empty in
    (Bisimulations.cardinal res,res,t)
