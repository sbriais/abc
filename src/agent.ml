...
let not_bisim = ref Bisimulations.empty;;

let bisim_empty p q d =
  not_bisim := bisim_add p q d (!not_bisim);
  Bisimulations.empty
...
object(self)
  ...
  method removeSon son =
    sons <- List.filter (function fils -> (fils#getElem <> son) || (not (fils#isLeaf))) sons
...
let trace_left = new arbreMutable (Tau,Nil)
let trace_right = new arbreMutable (Tau,Nil)

(* p and q must be in standard form *)
let rec add env s_comm w_comm trace_p trace_q p q d r =
  if bisim_neq p q d then (bisim_empty p q d)
  else
    ...
    if (bisim_exists p q d r) then r 
    else 
      if (compare p q = 0) then bisim_add p q d r
      else
	let ar_p = arity p and ar_q = arity q in
	  if (ar_p = 0) && (ar_q = 0) then
		...
		let r' = matches env s_comm w_comm trace_p trace_q pc qcw d (bisim_add p q d r) in
		  if Bisimulations.is_empty r' then (bisim_empty p q d)
		  else
		    begin
			...
			let r'' = matches env s_comm w_comm trace_q trace_p qc pcw d r' in
			  if Bisimulations.is_empty r'' then (bisim_empty p q d)
			  else
			    begin
			      r''
		...
		begin
		  ...
		  and (nq,q') = split_abstraction env 0 q in
		    ...
		    and q'' = substitute_agent sigma q' in
		      add env s_comm w_comm trace_p trace_q p'' q'' d (bisim_add p q d r)
		end
	      else
		...
		begin
		  ...
		  and (nq,vq,q') = split_concretion env 0 q in
			...
			and vq' = List.map sigma vq in
			  ...
			  else (bisim_empty p q d)
		      end
		    else (bisim_empty p q d)
		end
	  else (bisim_empty p q d)
and matches env s_comm w_comm trace_p trace_q p_comm q_comm d  r =
  if Bisimulations.is_empty r then 
	...
	begin
	  ...
	  let trace_p' = trace_p#newSon (alpha',p') in
	    (try
	       Commitments.iter 
		 (function (n,beta,q) ->
			...
			if (compare_action alpha' (substitute_action sigma_m 0 beta) = 0) then 
			  begin
			    ...
			    let r' = add env s_comm w_comm trace_p' (trace_q#newSon (alpha',q')) p' q' d' r in
			      if (Bisimulations.is_empty r') then ()
			      else 
				begin
				  trace_q#removeSon (alpha',q');
				  raise (FoundB(r'))
				end
			  end
			else ()
	     ...
	     with
		 FoundB(r') -> 
		   trace_p#removeSon (alpha',p');
		   matches env s_comm w_comm trace_p trace_q (Commitments.remove (m,alpha,p) p_comm) q_comm d r')
	end

let clear_all () =
  clear strong_tbl;
  clear weak_tbl;
  not_bisim := Bisimulations.empty;
  trace_left#clear;
  trace_right#clear

let eq env p q d =
  clear_all ();
  let res = add env commitments commitments trace_left trace_right (standard_form env 0 p) (standard_form env 0 q) d Bisimulations.empty in
    (Bisimulations.cardinal res,res)

let weq env p q d =
  clear_all ();
  let res = add env commitments weak_commitments trace_left trace_right (standard_form env 0 p) (standard_form env 0 q) d Bisimulations.empty in
    (Bisimulations.cardinal res,res)
